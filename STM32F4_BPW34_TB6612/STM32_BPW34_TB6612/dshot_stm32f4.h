...
class Stm32F4Dshot : public Stm32Dshot {
    ...
    private:
        ...
        static const uint32_t RCC_AHB1PERIPH_DMA1 = 0x00200000;
        ...
        static void timDmaCmd(TIM_TypeDef *timer, const uint16_t TIM_DMASource, const FunctionalState newState)
        {
            timer->DIER = 
                newState == DISABLE ? 
                timer->DIER & ~(uint16_t)TIM_DMASource :
                timer->DIER | TIM_DMASource;
        }
                ...
                const uint8_t polarity_shift2)
        {
            ...
            m_pacerTimer->CR1 = (m_pacerTimer->CR1 & (uint16_t)~TIM_CR1_CEN) | TIM_CR1_CEN;

            RCC_AHB1PeriphClockEnable(RCC_AHB1PERIPH_DMA1);
            ...
            // Set DMA channel to 2 for TIM4
            DMAy_Streamx->CR &= ~DMA_SxCR_CHSEL;
            DMAy_Streamx->CR |= (2 << DMA_SxCR_CHSEL_Pos);
            ...
            m_pacerTimer->CR2 = m_pacerTimer->CR2 & (uint16_t)~cr2_ois;

            *ccmr = (*ccmr & (uint16_t)~ccmr_oc & (uint16_t)~ccmr_cc) |
                (TIM_OCMODE_TIMING << mode_shift);

            m_pacerTimer->CCER = (m_pacerTimer->CCER & (uint16_t)~ccer_cc_e & (uint16_t)~ccer_ccp) |
        ...
        virtual void dmaInit(const uint32_t outputFreq) override
        {
            ...
            m_pacerTimer->CR1 = (m_pacerTimer->CR1 & ((uint16_t)(~(TIM_CR1_DIR | TIM_CR1_CMS)))) |
            ...
            m_pacerTimer->ARR = outputARR;
            m_pacerTimer->PSC = 0;
            m_pacerTimer->RCR = 0;
            m_pacerTimer->EGR = 0x0001;          

            // Reinitialize pacer timer for output
            m_pacerTimer->ARR = outputARR;

            // Use TIM4 CH3 (PB8) -> DMA1 Stream7 Channel2
            initPort(0, TIM_DMA_CC3, DMA1_Stream7, 22,  DMA1_Stream7_IRQn,
                    &m_pacerTimer->CCR3, &m_pacerTimer->CCMR2, TIM_CCER_CC3E,
                    TIM_CCMR2_OC3M, TIM_CCMR2_CC3S, TIM_CCER_CC3P,
                    TIM_CCER_CC3NP, TIM_CR2_OIS3, 0, 0, 0, 0);

            // Use TIM4 CH2 (PB7) -> DMA1 Stream6 Channel2
            initPort(1, TIM_DMA_CC2, DMA1_Stream6, 16, DMA1_Stream6_IRQn,
                    &m_pacerTimer->CCR2, &m_pacerTimer->CCMR1, TIM_CCER_CC2E,
        ...
        virtual void dmaUpdateComplete(void) override
        {
            ...
            timDmaCmd(m_pacerTimer, m_pacerDmaMask, ENABLE);
        }
        ...
        void handleDmaIrq(const uint8_t portIndex)
        {
            ...
            timDmaCmd(m_pacerTimer, port->dmaSource, DISABLE);
        ...
        void addMotor(const uint8_t pin, const uint8_t portIndex)
        {
            ...
            // Set DMA peripheral address to GPIO BSRR for this port
